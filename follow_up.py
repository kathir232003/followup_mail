...
def get_sent_message_details(sender_email, recipient_email, headers):
    """Get details of the most recent sent message to a recipient"""
    try:
        # Get the most recent sent message to this recipient
        ...
        if sent_messages_response.status_code == 200:
            ...
            if sent_data.get('value'):
                message_info = sent_data['value'][0]
                return {
        ...
        print(f"Error getting sent message details: {e}")
    return None
...
def send_emails():
    ...
    if not subject_template or not message_template.strip():
        messagebox.showwarning("Warning", "Please fill email subject and message.")
        return
    ...
    for index, row in df.iterrows():
        ...
        update_status(f"Sending email to: {recipient_email}", "blue")
...
def preview_email():
    ...
    try:
        ...
        email_type_indicator = " (Follow-up Reply)" if is_follow_up_email else " (New Email)"
...
frame_left = tk.Frame(main_frame, width=600)
frame_left.pack(side="left", fill="both", expand=True, padx=(0, 10))
frame_left.pack_propagate(False)

# Right frame for preview
frame_right = tk.Frame(main_frame, width=600)
...
tk.Label(subject_frame, text="Email Subject Template:").pack(side="left", pady=2)
...
tk.Label(message_frame, text="Email Message Template:", anchor="nw").pack(side="left", pady=2)
...
# Show initial thread status
if message_threads:
    thread_status_label.config(text=f"Threads: {len(message_threads)} available")
